...
export async function GET(request: Request) {
  ...
  return apiJson(request, { profile: { username: target, displayName: account[0].displayName, bio: person[0]?.bio ?? "", school: person[0]?.school ?? "", graduationYear: person[0]?.graduationYear ?? null, avatarColor: person[0]?.avatarColor ?? "indigo", selectedSubjects: safeJson(account[0].selectedSubjects, []), subjectLevels: safeJson(account[0].subjectLevels, {}), postCount: Number(postCount[0]?.count ?? 0), followerCount: Number(followerCount[0]?.count ?? 0), followingCount: Number(followingCount[0]?.count ?? 0), followed: relationship.length > 0, own: target === user.email } });
}

export async function PATCH(request: Request) {
  const access = await requireCommunityUser(request); if ("response" in access) return access.response;
  const { db, user } = access; const payload = await request.json() as { bio?: string; school?: string; graduationYear?: number | null; avatarColor?: string };
  const bio = String(payload.bio ?? "").trim().slice(0, 300); const school = String(payload.school ?? "").trim().slice(0, 80); const year = payload.graduationYear === null || payload.graduationYear === undefined || payload.graduationYear === 0 ? null : Number(payload.graduationYear); const avatarColor = colors.includes(String(payload.avatarColor)) ? String(payload.avatarColor) : "indigo";
  ...
  await db.insert(communityProfiles).values({ userEmail: user.email, bio, school, graduationYear: year, avatarColor }).onConflictDoUpdate({ target: communityProfiles.userEmail, set: { bio, school, graduationYear: year, avatarColor, updatedAt: new Date().toISOString() } });
