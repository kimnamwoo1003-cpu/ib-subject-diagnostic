...
export async function GET(request: Request) {
  ...
  return apiJson(request, { attempts: rows.map((row) => ({ ...row, topicBreakdown: safeJson(row.topicBreakdown, []), mistakes: safeJson(row.mistakes, []) })) });
}
...
export async function POST(request: Request) {
  ...
  if (!payload.subjectId || !payload.subjectName || !payload.level || !payload.paperId || !payload.paperName || !payload.mode) {
    return apiJson(request, { error: "Incomplete attempt data" }, { status: 400 });
  }
  ...
  if (payload.mode === "monthly" && !profile?.premium) {
    return apiJson(request, { error: "Premium access required" }, { status: 403 });
  }
  if (!profile?.premium) {
    const existing = await db.select({ id: testAttempts.id }).from(testAttempts).where(eq(testAttempts.userEmail, user.email)).limit(1);
    if (existing.length) return apiJson(request, { error: "Free accounts can complete one test only. Premium access is required for another attempt." }, { status: 403 });
  }
  ...
  return apiJson(request, { attempt }, { status: 201 });
}
