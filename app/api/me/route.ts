...
export async function GET(request: Request) {
  try {
    ...
    return apiJson(request, { user: { email: user.email, displayName: user.displayName, isAdmin: user.isAdmin }, premium: Boolean(profile?.premium), selectedSubjects: safeJson<string[]>(profile?.selectedSubjects ?? "[]", []), attempts: attempts.map((attempt) => ({ ...attempt, topicBreakdown: safeJson(attempt.topicBreakdown, []), criteriaBreakdown: safeJson(attempt.criteriaBreakdown, []), questionIds: safeJson(attempt.questionIds, []), difficultyTrail: safeJson(attempt.difficultyTrail, []), mistakes: safeJson(attempt.mistakes, []) })) });
  } catch (error) {
