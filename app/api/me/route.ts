...
export async function GET() {
  try {
    ...
    return Response.json({ user: { email: user.email, displayName: user.displayName, isAdmin: user.isAdmin }, premium: Boolean(profile?.premium), selectedSubjects: safeJson<string[]>(profile?.selectedSubjects ?? "[]", []), attempts: attempts.map((attempt) => ({ ...attempt, topicBreakdown: safeJson(attempt.topicBreakdown, []), criteriaBreakdown: safeJson(attempt.criteriaBreakdown, []), questionIds: safeJson(attempt.questionIds, []), difficultyTrail: safeJson(attempt.difficultyTrail, []), mistakes: safeJson(attempt.mistakes, []) })) });
  } catch (error) {
    console.error("Account initialization failed", error);
    return Response.json({ error: "Account storage is temporarily unavailable." }, { status: 500 });
  }
}
