...
export default function DiagnosticClient({ initialName }: { initialName: string }) {
  ...
  const chooseSubject = (id: string) => {
    ...
    if (!next) return;
    const nextLevel: Level = next.levels.includes("HL") ? "HL" : "SL";
  ...
  const chooseTopic = (code: string, index: number) => {
    void index;
    setSelectedTopicCodes((currentCodes) => currentCodes.includes(code) ? (currentCodes.length === 1 ? currentCodes : currentCodes.filter((item) => item !== code)) : [...currentCodes, code]);
  };
  ...
  const startTest = () => {
    ...
    const count = testMode === "monthly" ? 24 : premium ? 16 : 8;
    ...
    const pool = buildQuestionPool(subject, level, paper, includedTopics, premium, codeLanguage, seed, previousIds);
    const first = pool.find((question) => question.difficultyLevel === 3) ?? pool.find((question) => question.difficulty === "Standard") ?? pool[0];
    const seconds = testMode === "monthly" ? Math.min(60 * 60, Math.max(30 * 60, count * 150)) : 0;
    setQuestionPool(pool); setTargetQuestionCount(count); setQuestions(first ? [first] : []); setAnswers({}); setCurrent(0); setAdaptiveLevel(3);
    setDifficultyTrail(first ? [first.difficulty] : []); setStartedAt(Date.now()); setTimeLimit(seconds); setTimeLeft(seconds); setSavedResult(null); setSaveError(""); setStage("test"); window.scrollTo({ top: 0 });
  };
  ...
  useEffect(() => {
    if (stage !== "test" || testMode !== "monthly" || timeLimit <= 0) return;
    const timer = window.setInterval(() => setTimeLeft((value) => {
      if (value <= 1) {
        window.clearInterval(timer);
        window.setTimeout(() => document.getElementById("monthly-auto-submit")?.click(), 0);
        return 0;
      }
      return value - 1;
    }), 1000);
  ...
  return <main className="app-shell">
    ...
    {stage === "setup" && <div className="page-container setup-page" style={{ "--subject": subject.color, "--subject-soft": subject.softColor } as React.CSSProperties}>
      ...
      <SetupBlock number="3" title="Select exactly what you want to test" subtitle="Choose any individual topic. Selecting B1 does not automatically include A1, A2 or A3." side={`${includedTopics.length} topics`}><div className="topic-list">{topics.map((topic, index) => { const included = selectedTopicCodes.includes(topic.code); return <button type="button" key={topic.code} className={`topic-row ${included ? "included" : ""}`} onClick={() => chooseTopic(topic.code, index)}><span className="topic-check">{included ? "✓" : ""}</span><strong>{topic.code}</strong><span>{topic.title}</span>{topic.level === "HL" && <b>HL only</b>}</button>; })}</div></SetupBlock>
      <SetupBlock number="4" title="Choose test mode" subtitle="Monthly Progress Test is timed and compares results with your previous month."><div className="tier-grid">
        <button className={`tier-card ${testMode === "diagnostic" ? "selected" : ""}`} onClick={() => setTestMode("diagnostic")}><span className="tier-top"><strong>{me?.premium ? "Deep diagnostic" : "Quick diagnostic"}</strong><em>{me?.premium ? "PREMIUM" : "FREE"}</em></span><p>{me?.premium ? "Up to 16 questions with detailed topic feedback" : "Up to 8 questions with a focused next step"}</p><ul><li>Paper-specific question types</li><li>Estimated grade and answer review</li></ul></button>
        <button className={`tier-card premium ${testMode === "monthly" ? "selected" : ""} ${!me?.premium ? "locked" : ""}`} disabled={!me?.premium} onClick={() => me?.premium && setTestMode("monthly")}><span className="popular">MONTHLY CHECK-IN</span><span className="tier-top"><strong>Monthly Progress Test</strong><em>PREMIUM</em></span><p>Up to 24 questions · adaptive 30–60 minute timer</p><ul><li>Previous-test comparison</li><li>Topic gains and remaining gaps</li><li>Speed and score change</li></ul></button>
      </div></SetupBlock>
      {saveError && <div className="inline-error">{saveError}</div>}<div className="start-panel"><div><strong>{subject.name} {level} · {paper.name}</strong><span>{includedTopics.length} topics · {paper.id === "concept" ? "adaptive concept MCQ" : testMode === "monthly" ? "timed monthly progress test" : "adaptive diagnostic test"}</span></div><button className="primary-button" disabled={!includedTopics.length || freeLocked} onClick={startTest}>{freeLocked ? "Free test already used" : `Start ${testMode === "monthly" ? "monthly test" : "diagnostic"}`} <span>→</span></button></div>
    </div>}
...
function SubjectOnboarding({ name, current, onSaved }: { name: string; current: string[]; onSaved: () => Promise<void> }) {
...
function QuestionCard({ question, answer, onAnswer }: { question: Question; answer: string; onAnswer: (value: string) => void }) {
  return <article className="question-card">
    ...
    {question.visual && <QuestionVisual type={question.visual}/>}
...
function QuestionVisual({ type }: { type: NonNullable<Question["visual"]> }) {
