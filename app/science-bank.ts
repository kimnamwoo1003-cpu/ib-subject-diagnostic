...
function computerScience(subject: Subject, paper: Paper, node: ScienceNode, v: number, d: number, language: "python" | "java"): Question {
  ...
  const starter = language === "python"
    ...
    : `public static List<Integer> validReadings(int[] values) {\n    List<Integer> result = new ArrayList<>();\n    // return valid values in ascending order without duplicates\n    return result;\n}`;
  const prompt = d <= 2
    ? `Trace the algorithm for input [7, 3, 7, -1, 5]. Show each state change and the final output. Values below 0 are invalid.`
    : d === 3
      ? `Complete the ${language === "python" ? "function" : "method"}. Accept only values from 0 to 100, remove duplicates and return the remaining values in ascending order.`
      : `Complete and test the ${language === "python" ? "function" : "method"}. Include normal, boundary, invalid and extreme tests, then state the time complexity of the dominant step.`;
  return {
    ...base(subject, paper, node, v, d, prompt, `Program specification: input may be empty and may contain duplicate, negative or greater-than-100 integer readings. The original input must not be modified.`, d <= 2 ? "Trace" : "Construct", d <= 2 ? 5 : d === 3 ? 8 : 12,
      [`Show every state change, branch and output for a trace.`, `Validate 0 and 100 as inclusive boundaries and reject other values.`, `Remove duplicates, sort the result and do not mutate the input.`, `Use normal, boundary, invalid and extreme tests; justify complexity from the implemented operations.`],
      ["final output only", "boundary excluded", "input mutated", "complexity asserted without reference to code"], "code"),
    starterCode: starter,
